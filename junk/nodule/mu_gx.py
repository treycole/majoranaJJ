...
Sx = int(Nx/2) #3 #length of either side of nodule
cutx = (Nx - 2*Sx) #width of nodule
cuty = 0 #4 #height of nodule
...
for i in range(steps):
    ...
    gammaxpi = gf(
        coor, ax, ay, NN, mu[i], NNb = NNb, Wj = Wj, Sx = Sx, cutx = cutx, cuty=cuty, alpha = alpha, delta = delta, phi = np.pi, gammax = gammax_i, V = V, periodicX = True
        )
...
plt.legend()
plt.savefig('oldgamfinder_juncwidth = {} SCwidth = {} V0 = {} nodwidthx = {} nodwidthy = {} .png'.format(Junc_width, SC_width, V0, 0, 0 ))
plt.show()
