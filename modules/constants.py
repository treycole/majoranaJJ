Jtoev = 1/(1.602176487e-19)     #Conversion of Joules to eV
evtoJ = 1.602176487e-19         #Conversion of eV to Joules
meVpK = 8.6173325e-2            #Kelvin into [meV]

hbar = 6.582119569e-16          #h/2π in [eV*s]
e0 = 1.602176634e-19            #electron charge in [C]
m0 = 9.1093837015e-31           #electron mass in [kg]
meff = 0.02*m0                  #effective mass
xi = ((hbar**2)*(e0)*(10**20)*(10**3))/(meff)  #hbar^2/meff in [meV A^2]
muB = 5.7883818060e-2            #[meV/T]
