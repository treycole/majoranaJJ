...
if PLOT != 'P':
    for i in range(gx.shape[0]):
        for j in range(mu.shape[0]):
            ...
            H = slfNRG.Junc_eff_Ham_gen(omega=0, Wj=Wj, nodx=0, nody=0, ax=ax, ay_targ=ay, kx=qx, m_eff=0.026, alp_l=alpha, alp_t=alpha, mu=mu[j], Vj=Vj, Gam=gx[i], delta=delta, phi=phi, iter=50,eta=0)
