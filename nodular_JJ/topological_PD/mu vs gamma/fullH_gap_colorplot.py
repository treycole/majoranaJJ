    ...
    for q in range(qx.shape[0]):
        ...
        for i in range(mu.shape[0]):
            ...
            if q == 0 or top_array[i] == 1:
                ...
                H_G0 = spop.HBDG(coor, ax, ay, NN, NNb=NNb, Wj=Wj, cutx=cutx, cuty=cuty, V=V, mu=mu[i], gammax=0, alpha=alpha, delta=delta, phi=phi, qx=qx[q], Tesla = False, Zeeman_in_SC = False, SOC_in_SC = True) #Matrix that consists of everything in the Hamiltonian except for the Zeeman energy in the x-direction
                H_G1 = spop.HBDG(coor, ax, ay, NN, NNb=NNb, Wj=Wj, cutx=cutx, cuty=cuty, V=V, mu=mu[i], gammax=1, alpha=alpha, delta=delta, phi=phi, qx=qx[q], Tesla = False, Zeeman_in_SC = False, SOC_in_SC = True) #Hamiltonian with ones on Zeeman energy along x-direction sites
