...
Vj = -30 #Amplitude of potential in junction region: [meV]
...
if PLOT != 'P':
    ...
    for i in range(mu.shape[0]):
        print(mu.shape[0]-i)
        gap, q_minimum = fndrs.gap_finder(coor, NN, NNb, ax, ay, mu[i], gx, Wj=Wj, cutx=cutx, cuty=cuty, Vj=Vj, alpha=alpha, delta=delta, phi=phi, steps_targ=5000)
        print("gap", gap)
        GAP.append(gap)
