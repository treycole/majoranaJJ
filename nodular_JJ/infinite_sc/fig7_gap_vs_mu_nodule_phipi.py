...
for i in range(local_min_idx.shape[0]):
    ...
    if gap[local_min_idx[i]]/delta < 0.02 and (Lx*kx_of_gap[local_min_idx[i]] <= 0.15 or abs(Lx*kx_of_gap[local_min_idx[i]] - np.pi) <= .138):
        num=num*-1
    if num==1:
...
art.set_edgecolor('k')
axs[0].plot([1,1], [-2,12], c='r', lw=1.5, zorder=3)
